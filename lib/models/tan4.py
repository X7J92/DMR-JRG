...
################################################################################计算最佳的提案所对应的融合特征#####################################################################
        ...
        for index_1 in non_zero_indices:
            # 提取特定索引的元素
            element = fused_h_reshaped[index_1[0], index_1[1], :, index_1[3], index_1[4]]
            # 填充到结果容器中
            results[index_1[0], index_1[1]] = element
            # 更新掩码：已填充的位置设置为 False
            mask[index_1[0], index_1[1], 0] = False
        # print(results.shape)torch.Size([2, 8, 512])
        mask_squeezed = torch.squeeze(mask, dim=2)
        results1 = results.view(8,batch_size,512)

        for index_2 in non_zero_indices_second:
            # 提取特定索引的元素
            element = fused_h_reshaped[index_2[0], index_2[1], :, index_2[3], index_2[4]]
            # 填充到结果容器中
            results_second[index_2[0], index_2[1]] = element
        # results_second = results_second.view(8,batch_size,512)


        for index_3 in non_zero_indices_last:
            # 提取特定索引的元素
            element = fused_h_reshaped[index_3[0], index_3[1], :, index_3[3], index_3[4]]
            # 填充到结果容器中
            results_last[index_3[0], index_3[1]] = element
        ...
        # results_c = results.permute(1, 0, 2).reshape(batch_size * 8, 512)

        results = results*0.4 + results_second*0.3 + results_last*0.3
        results1 = results.view(8, batch_size, 512)


        sentence_mask1_bool=sentence_mask1.view(batch_size*8).bool()

        results_cc=results.view(batch_size*8,512)

        results_c=results_cc[sentence_mask1_bool]
        ...
        # targets = torch.zeros(batch_size, tmp_shape[1], tmp_shape[-2] * tmp_shape[-1], 3).cuda()
        #
        # # 使用scatter填充1到最大值的位置
        # for i in range(3):  # 对于前三个最大值
        #     targets.scatter_(2, indices[..., i:i + 1], 1)
        #
        # # 重新调整形状以匹配原始形状，并将其与map_mask相乘
        # targets = torch.reshape(targets, tmp_shape + (3,)) * map_mask.unsqueeze(-1)
        #
        # # 根据非零索引处理其他张量
        # non_zero_indices = torch.nonzero(targets, as_tuple=False)
        #
        # # 调整 fused_h 的形状
        # fused_h_reshaped = fused_h.view(batch_size, 8, 512, 32, 32)
        #
        ...
        #
        # # 初始化结果容器，现在考虑到前三个最大值
        # results = torch.zeros(batch_size, 8, 512, 3).cuda()
        #
        # mask = torch.ones(batch_size, 8, 3, dtype=torch.bool).cuda()
        #
        # for index in non_zero_indices:
        #     # 提取特定索引的元素，注意处理三个通道
        #     for i in range(3):
        #         if index[-1] == i:  # 检查通道索引
        #             element = map_h_c[index[0], index[1], :, index[3], index[4]]
        #             results[index[0], index[1], :, i] = element
        #             mask[index[0], index[1], i] = False
        #
        # # 调整结果的形状
        # results = results.permute(1, 0, 2, 3)  # 调整为(8, batch_size, 512, 3)
        #
        # mask_squeezed = torch.squeeze(mask, dim=2)
...
############################################################################################################################
        ...
        #TF
        vl_src = torch.cat([tgt_src_v,results1.cuda()], dim=0)
...
#######################################################################################################################################################################
        ...
        # print(txt_g.shape)torch.Size([8, 4, 512])
...
###################################################################################################
        ...
        # print(sims)
 ####################################################################################################################################################
        # print(sentence_mask1.shape)torch.Size([16, 8, 1])
        # print(results.shape)txt_g.  shape([8, 16, 512])
        # print(txt_g.shape)txt_g.shape  shape([8, 16, 512])
        # results_c=results.permute(1,0,2).reshape(batch_size*8,512)
        txt_g_c=txt_g.permute(1,0,2).reshape(batch_size*8,512)
        # sentence_mask1_bool=sentence_mask1.view(batch_size*8).bool()
        # results_c=results_c[sentence_mask1_bool]
        txt_g_c=txt_g_c[sentence_mask1_bool]
        results_c = results_c / results_c.norm(dim=-1, keepdim=True)
        txt_g_c = txt_g_c / txt_g_c.norm(dim=-1, keepdim=True)
        sims2 = torch.matmul(results_c, txt_g_c.T)
        # sims2[torch.isnan(sims2)] = 0
        # print( sentence_mask1_bool)
        # print(sims2)
        # print(sims2)
#############################################################################################################################################
...
##############################################################################revse#########################################################################################3
        ...
        weights=None
        return prediction, map_mask, sims, self.logit_scale,sims2, self.logit_scale2, jj, weight_3, words_logit, ids_list, weights, words_mask1,overlaps_tensor_z,p_values_tensor_z,overlaps_tensor_f,p_values_tensor_f
