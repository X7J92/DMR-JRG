...
class TAN(nn.Module):
    ...
    def forward(self, textual_input, textual_mask, sentence_mask, visual_input,duration,weights_list,ids_list):
        ...
        # visual_output_g=self.gg(visual_output)
        visual_output_g= vis_h
        # print(visual_output_g.shape)
...
##############################################################################revse#########################################################################################3
        ...
        weights=None
        return prediction, map_mask, sims, self.logit_scale,sims2, self.logit_scale2,sims3,self.logit_scale3, jj, weight_3, words_logit, ids_list, weights, words_mask1,overlaps_tensor_z,p_values_tensor_z,overlaps_tensor_f,p_values_tensor_f, vg_hs_video,vg_hs_t,vg_hs_v
